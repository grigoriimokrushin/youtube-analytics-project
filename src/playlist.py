...
class PlayList(YouTubeMixin):
    ...
    def __init__(self, playlist_id: str):
        ...
        self.__playlist = self.get_service().playlistItems().list(playlistId=self.__playlist_id, part='contentDetails', maxResults=50).execute()
        self.__video_ids: list[str] = [video['contentDetails']['videoId'] for video in self.__playlist['items']]
        self.__video_response = self.get_service().videos().list(part='snippet,contentDetails,statistics', id=','.join(self.__video_ids)).execute()
        self.__channel_id = self.__video_response['items'][0]['snippet']['channelId']
        self.__playlists = self.get_service().playlists().list(channelId=self.__channel_id, part='contentDetails,snippet', maxResults=50).execute()
